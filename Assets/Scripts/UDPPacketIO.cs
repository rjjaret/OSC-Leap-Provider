...
using UnityEngine;
  ...
  public class UDPPacketIO : MonoBehaviour 
  {
    ...
    void Start() 
    {
      ...
      init(hostIP, remotePort, localPort, true);
  	}
  	

    // for initializing as a sender only
  	public void init(string hostIP, int remotePort)
    {
      init(hostIP, remotePort, 0, false);
  	}


  	private void init(string hostIP, int remotePort, int localPort, bool isListening){

      IsListener = isListening;
    ...
    public bool IsListener { get => isListener; set => isListener = value; }
}
